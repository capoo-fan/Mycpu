...
module IF_stage(
    ...
    // 反压到 PC
    output wire        full,
    // ICache miss refill 接口 (连接 icache_refill)
    output wire        rd_req,
...
always @(*) begin
    nstate = state;
    case (state)
        FSM_IDLE:        nstate = trigger_miss ? FSM_MISS_REQ : FSM_IDLE;
        FSM_MISS_REQ:    nstate = rd_rdy ? FSM_MISS_REFILL : FSM_MISS_REQ;
        FSM_MISS_REFILL: nstate = (ret_valid && ret_last[0]) ? FSM_RECOVERY : FSM_MISS_REFILL;
        FSM_RECOVERY:    nstate = FSM_DONE;
        FSM_DONE:        nstate = s3_stall ? FSM_DONE : FSM_IDLE;
...
always @(posedge clk) begin
    if (s2_valid && !s2_hit && state == FSM_IDLE) begin
...
always @(posedge clk) begin
    if (!resetn) begin
        ...
        if (state == FSM_MISS_REQ && nstate == FSM_MISS_REFILL) begin
            refill_beat <= 2'b0;
            refill_line <= 128'b0;
        end

        // MISS_REFILL: 收集重填数据
        if (state == FSM_MISS_REFILL && ret_valid) begin
            case (refill_beat)
                2'd0: refill_line[31:0]   <= ret_data;
                2'd1: refill_line[63:32]  <= ret_data;
                2'd2: refill_line[95:64]  <= ret_data;
                2'd3: refill_line[127:96] <= ret_data;
            endcase
            refill_beat <= refill_beat + 2'd1;
        end

        // RECOVERY: 写入 Cache + 锁存 refill_data_reg
        if (state == FSM_RECOVERY) begin
