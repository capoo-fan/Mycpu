...
# SRAM read-interface timing guidance
#
# Keep the CPU clock unchanged.  These datapath-only constraints give the
# placer/router explicit priorities for the two parts of an asynchronous SRAM
# read: launching address/control to the package pins, and returning read data
# from the package pins to the first architectural capture registers.
#
# Do not extend the return constraint to downstream CPU registers.  Doing so
