...
归约、位处理、校验和算术题。模块分为两层：

- 独立核心：接口精简，适合直接接入 ALU、EX 阶段或自定义加速器；
- `contest_function_unit`：统一 `valid/ready` 请求响应接口，适合比赛现场快速复用。
...
| `arithmetic_map_adapter.v` | 原算术入口到 Map 模板 | 固定第二操作数 |
| `function_map_adapter.v` | 全函数入口到 Map 模板 | 固定第二操作数和控制字段 |
...
`in_data` 对应 `operand_a`，其余输入使用参数常量。若两个操作数或位域控制都来自
内存，应直接实例化 `contest_function_unit`，由上层读取并锁存所有请求字段。
