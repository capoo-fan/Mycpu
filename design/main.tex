...
PLL 使用 Clocking Wizard 由 50\,MHz 输入产生 148\,MHz CPU 时钟和 20\,MHz 辅助时钟\upcite{xilinxclk}。复位由外部按钮或 PLL 未锁定异步置位，再在 CPU 时钟域两级同步释放。UART 波特率为 115200，接收与发送模块的 \code{ClkFrequency} 参数均取 \code{CPU_CLK_FREQ}。
...
\begin{table}[H]
  ...
  \begin{tabular}{c c c c c c}
    ...
    \midrule
    148\,MHz & \grayfill{33ms} & \grayfill{68ms} & \grayfill{182ms} & \grayfill{4ms} & \grayfill{287ms} \\
    \bottomrule
  \end{tabular}
\end{table}
