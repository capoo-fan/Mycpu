...
module issue_special_tb;
  ...
  task check_local_wait_pipeline;
    begin
      ...
      // An independent lane1 load is consumed with the older ALU and steered
      // onto the physical lane0 memory datapath. Its destination is mirrored
      // as the sole non-forwardable EX producer.
      front_valid_1 = 1'b1;
      inst_0 = make_addi(5'd10, 5'd0);
      inst_1 = make_ld_w(5'd13, 5'd4);
      check_issue(1'b1, 1'b1, "lane1 load steers into memory lane");
      tick();
      check_local_wait(1'b1, 5'd13,
                       "steered lane1 load creates physical lane0 wait");
  ...
  initial begin
    ...
    // A lane1 Store can steer only when all of its sources are ready.
    front_valid_1 = 1'b1;
    inst_0 = make_addi(5'd10, 5'd0);
    inst_1 = make_st_w(5'd12, 5'd5);
    set_producer(0, 1'b0, 5'd12);
    check_issue(1'b1, 1'b0, "unready lane1 Store remains in IBuffer");
    clear_producers();
    check_issue(1'b1, 1'b1, "ready lane1 Store steers into memory lane");
