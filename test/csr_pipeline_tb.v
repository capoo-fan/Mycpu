...
module csr_pipeline_tb;
  ...
  begin
    ...
    if (resetn && (|debug_we) && (debug_wnum != 5'b0))
    begin
      if (event_count > 8 || debug_pc !== expected_pc[event_count] ||
          debug_wnum !== expected_reg[event_count] ||
          debug_wdata !== expected_data[event_count])
      begin
        $display("FAIL event %0d: pc=%h reg=%0d data=%h",
                 event_count, debug_pc, debug_wnum, debug_wdata);
