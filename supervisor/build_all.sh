...
echo "[BUILD] LA32R disassembly for performance tests"
"$PYTHON" "$ROOT_DIR/utility/disassemble_utests.py" \
    --elf "$BUILD_DIR/kernel/auto/kernel.elf" \
    --kernel-bin "$BUILD_DIR/kernel/auto/kernel.bin" \
    --output-dir "$BUILD_DIR/utility/disasm" \
    --nm "$CROSS_NM" \
