...
module branch_judge #(
  ...
  wire [31:0] target_base = SIMPLE_ONLY ? pc :
       ((br_op == `BR_JIRL) ? rkd_value : pc);

  assign is_bj = (br_op != `BR_NONE);

  wire simple_taken = (br_op == `BR_BEQ)  &&  rj_eq_rkd          ||
  ...
  assign real_taken = (simple_taken ||
                       (!SIMPLE_ONLY &&
                        ((br_op == `BR_JIRL) || (br_op == `BR_BL)))) &&
                      is_bj;
