...
└── utility/
    ├── disasm/        # 四个性能程序的 BIN 和 LA32R 反汇编
...
`build_all.sh` 还会按 `utest_symbols.txt` 中的链接地址，从 `kernel.bin` 精确截取
STREAM、MATRIX、CryptoNight 和 MIXED 四段二进制，并用 LA32R `objdump`
生成 `build/utility/disasm/*.disasm`。`MANIFEST.txt` 记录每段的起止地址、字节数和
SHA-256，用于确认反汇编与实际仿真镜像一致。
