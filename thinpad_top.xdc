...
# FAST output slew advances address, byte-enable, CE and OE arrival at the
# SRAM without changing the CPU clock period.
...
set sram_mem_capture_cells [get_cells -hier -quiet -filter {
    NAME =~ u_cpu/u_mem/ms_rdata_buf_reg[*]
}]
set sram_mem_capture_pins [get_pins -of_objects $sram_mem_capture_cells \
    -filter {REF_PIN_NAME == D}]

set base_sram_icache_capture_cells [get_cells -hier -quiet -filter {
    NAME =~ u_cpu/u_icache_refill/icache_ret_data_r_reg[*]
}]
set base_sram_icache_capture_pins \
    [get_pins -of_objects $base_sram_icache_capture_cells \
        -filter {REF_PIN_NAME == D}]

set sram_read_launch_budget_ns     12.000
set base_sram_return_budget_ns     6.000
set ext_sram_return_budget_ns      5.000

set_max_delay -datapath_only $sram_read_launch_budget_ns \
...
set_max_delay -datapath_only $base_sram_return_budget_ns \
    -from $base_sram_read_data_ports -to $base_sram_icache_capture_pins
set_max_delay -datapath_only $ext_sram_return_budget_ns \
    -from $ext_sram_read_data_ports -to $sram_mem_capture_pins

set_property LOC SLICE_X44Y49 [get_cells -hier -quiet -regexp \
    {u_cpu/u_inst_buffer/head_reg\[0\]_rep__0}]
set_property LOC SLICE_X45Y49 [get_cells -hier -quiet -regexp \
    {u_cpu/u_inst_buffer/front_bus_0_g3_reg\[42\]}]

set_property LOC SLICE_X29Y62 [get_cells -hier -quiet -regexp \
    {u_cpu/u_mem/front_bus_0_g5\[40\]_i_1}]


set_property LOC SLICE_X32Y33 [get_cells -hier -quiet -regexp \
    {u_cpu/u_inst_buffer/front_raddr2_1_hot_r\[2\]_i_1}]
