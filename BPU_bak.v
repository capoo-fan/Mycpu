...
module BPU (
  ...
  // 根据 bpu_track 统计结果，仅保留执行次数 >= 2 的分支/跳转 PC
  localparam integer B_VALID_PC_NUM = 2;
  localparam integer J_VALID_PC_NUM = 1;

  reg [31:0] b_valid_pc [0:B_VALID_PC_NUM-1];
  reg [31:0] j_valid_pc [0:J_VALID_PC_NUM-1];
  ...
  wire if_pc_in_valid = (if_pc == b_valid_pc[0]) ||
       (if_pc == b_valid_pc[1]) ||
       (if_pc == j_valid_pc[0]);

  wire ex_pc_in_valid = (ex_pc == b_valid_pc[0]) ||
       (ex_pc == b_valid_pc[1]) ||
       (ex_pc == j_valid_pc[0]);
